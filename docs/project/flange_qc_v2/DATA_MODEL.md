...
`shadow` is an internal framework and contract term for running the app beside
the real human QC process. It means parallel-QC, review-only evidence collection;
it does not mean a fake application, a simulated QC replacement, or production
approval. Operator-facing HMI text should use `Parallel QC`, `Model Assist`, and
`Review-only` wording, while existing contract fields such as `shadow_mode`,
`shadow_decision`, and `/detector/shadow/*` remain stable for compatibility.
...
- Model artifact manifests must remain parallel-QC/review-only until model approval and
...
- Model artifact manifests from MLOps provider or local parallel-QC evaluation.
- Dataset manifests and evaluation reports from MLOps provider or local parallel-QC
  evaluation.
...
return behavior. This local audit path is replay/parallel-QC evidence only and does
...
boundary corners and calibration scale. The bootstrap parallel-QC path uses four
...
Bootstrap parallel-QC decision evaluation lives in:

```text
apps/flange_qc_v2/decision_engine.py
```

The current decision engine can emit parallel-QC `PASS`, `NG`, or `BLOCKED` evidence
...
production-enabled state. Parallel-QC `PASS`/`NG`, `ASSIST`, and `NOT_EVALUATED`
...
`sop_decision_engine=parallel_qc_implemented_requires_qc_sop_approval` rather than
`not_implemented`. This means the parallel-QC SOP logic is present for replay,
...
`MARK_FALSE_POSITIVE` through the existing parallel-QC feedback contract. The phase
...
current inspection through `/feedback`. This UI remains no-camera parallel-QC
...
The current feedback contract accepts operator/domain notes as parallel-QC evidence
...
parallel-QC `PASS`, `ASSIST`, or `BLOCKED` evidence. Dark, blurry, or severely
occluded frames fail closed as `BLOCKED`; moderate occlusion emits `ASSIST` for
QC review. Passing evidence remains review-only and carries
...
parallel-QC SOP evidence: relevant observations carry matched labels,
...
outside the primary QC tablet viewport. It displays lane summaries for parallel-QC
...
not carry raw frames, datasets, model binaries, inference output, camera configuration, or
final QC PASS/NG authority. The HMI renders this payload as detector bridge
status with Parallel QC / Review-only wording so operators can see metadata
readiness without opening raw JSON or changing backend decision authority. The
route name stays `/detector/shadow/status` because it is an existing internal
compatibility contract for the parallel-QC lane.

`POST /detector/shadow/observations` is the metadata-only dry-run path for
parallel-QC detector integration. It accepts detector request metadata plus sanitized
...
for parallel-QC validation only and does not approve model execution or production
authority.
